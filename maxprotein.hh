...
std::unique_ptr<FoodVector> filter_food_vector(const FoodVector& source,
    ...
    int sizeCounter = 0;
    std::unique_ptr<FoodVector>filteredFoodZeroCal(new FoodVector);
    
    for (int i=0;i<total_size;i++)
    {
        if (source[i]->kcal()>0 && source[i]->kcal() >= min_kcal && source[i]->kcal() <= max_kcal)
        {
            filteredFoodZeroCal->push_back(source[i]);
            
        }
    }


    return filteredFoodZeroCal;
}
...
std::unique_ptr<FoodVector> greedy_max_protein(const FoodVector& foods,
    ...
    int result_cal = 0;
    int result_protein=0;
    auto f = todo[0]; // creating a temp to that will hold the best food
    
    while (!todo.empty())

    {
        int toDoMaxProtein = 0;
        int maxPLocation = 0;
        ...
        {
            if (toDoMaxProtein < todo[i]->protein_g())
            {
                toDoMaxProtein = todo[i]->protein_g();
                f = todo[i]; // assign the best food to the temp
                maxPLocation = i;
                c = todo[i]->kcal();
        ...
        todo.erase(todo.begin() + maxPLocation);
...
std::unique_ptr<FoodVector> exhaustive_max_protein(const FoodVector& foods,
    ...
    int best_total_cal = 0; 
    int best_total_prot = 0;


    for (uint64_t bits = 0; bits < pow(2, n);bits++)
    {
        FoodVector * candidate = new FoodVector;
        int candidateKCal = 0;
        int candidateProtein = 0;
        for (int j = 0;j < n;j++)
        {
            if (((bits >> j) & 1) == 1)
            {
        ...
        sum_food_vector(candidateKCal, candidateProtein, *candidate);
        if (candidateKCal <= total_kcal)
        {
            if (best->empty() || candidateProtein > best_total_prot)
            {
                best = std::unique_ptr<FoodVector>(candidate);
                sum_food_vector(best_total_cal, best_total_prot, *best);
            }
        }
    }
